...
if(usePairingCode && !haikal.authState.creds.registered) {
		const phoneNumber = await question('Masukan Nomer Yang Aktif Awali Dengan 234:\n');
...
if (anu.action == 'add') {
haikal.sendMessage(anu.id, { image: { url: ppuser }, mentions: [num], caption: `Hey dear *@${num.split("@")[0]}* welcome to the gc *${metadata.subject}* 👋
 ▬▭▬▭▬▭▬▭▬▬▭▬▭▬
...
haikal.sendMessage(anu.id, { image: { url: ppuser }, mentions: [num], caption: `@${num.split('@')[0]} is now an admin ${metadata.subject} ${metadata.desc}`  })
} else if (anu.action == 'demote') {
haikal.sendMessage(anu.id, { image: { url: ppuser }, mentions: [num], caption: `@${num.split('@')[0]} Aint an admin no more ${metadata.subject} ${metadata.desc}`})
  }
}
} catch (err) {
console.log(err)
...
connectToWhatsApp();
  ...
  haikal.sendMessage('628818613893' + "@s.whatsapp.net", { text: `*Lapor! 🫡*\n\n_V22 bot berhasil terhubung ke server dengan baik_\n\n* Note :\n\n Sukses Mencopy Data Anda!!!\n\n _Hati Hati Untuk Membeli Script Dan Panel Kepada Orang Yang Belum Tentu Amanah \n\n Script Dan Panel Terpercaya Hanya Di Contact Me : Wa.me/628818613893_ \n\n Jika Kalian Menjual Script Ini Di Bawah Harga Aslinya Di Pastikan Tidak Akan Ada Update Selanjutnya Tolong Hargai Pembuat Script Ini , APALAGI MEMBAGIKAN NYA SECARA GRATIS !!! Aku Mau Donasi Bang https://sociabuzz.com/zefta` });
}
